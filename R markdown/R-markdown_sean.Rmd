...
```
We chose to use the CART (Classification and Regression Trees) method of imputation. Blue represents the distribution of the original data, while red represents the distribution of imputed data.

```{r CART imputation, echo=FALSE, fig.align='center', fig.height=3, fig.width=4, fig.gap = 'CART-imputed values for sqft'}
cart <- readRDS('imputed_df_cart.rds')
densityplot(cart)
```
Below we also summarize 
```{r histogram, echo=FALSE, fig.align='center', fig.height=5, fig.width=5}
...
```
```{r boxplot, echo=FALSE, fig.align='center', fig.height=5, fig.width=5}
...
```
```{r ggplot,echo=FALSE, fig.align='center', fig.height=3, fig.width=4}
par(mfrow=c(1,1))
ggplot(df, aes(type)) + geom_bar(fill = 'red') + theme(axis.text.x = element_text(angle = 90, vjust = 0.5, hjust=1))
```
```{r corrplot, echo=FALSE, fig.align='center', fig.height=5, fig.width=5}
