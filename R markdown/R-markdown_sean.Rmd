...
We found that the 'sqft' variable was missing approximately 30% of its values.  To test whether the missing data was MCAR, we compared whether certainother variables were associated with the missing sqft values. Using Chi-squared and Kruskal Wallis tests for categorical and numerical variables, respectively, we determined that all variables had significantly correlation (p<0.01) with sqft missingness. Thus, the data is not MCAR, and we cannot simply remove observations where sqft is missing without introducing bias. 

```{r missingness missingness correlation missingness imputation, echo=FALSE}
...
```
We chose to use the CART (Classification and Regression Trees) method of imputation (Figure 1). Blue represents the distribution of the original data, while red represents the distribution of imputed data.

```{r CART imputation, echo=FALSE, fig.align='center', fig.height=3, fig.width=4, fig.cap = 'CART-imputed values for sqft'}
cart <- readRDS('imputed_df_cart.rds')
densityplot(cart)
```
Below, we summarize the distribution of predictor and target variables (Figure 2 and Figure 3). Note the significant right skew within each variable. Additionally, each variable also contained many outliers. The data is therefore unlikely to be fit for any sort of parametric machine-learning algorithm without performing significant transformations.

```{r histogram, echo=FALSE, fig.align='center', fig.height=4, fig.width=4, fig.cap = 'Distribution of predictor and target variables'}
...
ggplot(df, aes(type)) + geom_bar(fill = 'red') + theme(axis.text.x = element_text(angle = 90, vjust = 0.5, hjust=1))
```
```{r corrplot, echo=FALSE, fig.align='center', fig.height=5, fig.width=5, fig.cap='Correlogram'}
...
#### **Decision Tree Model**\
The decision tree model was tuned with different value of cp, 0.001 is used for best model performance after tuning.
...
attributes(Extreme_Gradient_Boosting_model)
Extreme_Gradient_Boosting_model
```
### **Model Selection**\
  For the four models, we chooses the extreme gradient boosting model as our final model for deployment. Mean Absolute Error (MAE) tells the average error of the variable we want to predict. Root Mean-Squared Error (RMSE) is similar with MAE but it is more useful when we want fewer larger errors more than a lot of small errors, we want the model be more stable thus we take RMSE as one of the evaluation matrix. R-Squared (R2) computes how much better the regression line fits the data than the mean line which can give us a overall score for the model.As you can see from the table below, all the models has similar RMSE, MAE and R2. For predicting house price, we want a model with lowest RMSE and MAE to keep the high accuracy of prediction. The extreme gradient boosting model has the highest R-Squared and the lowest Mean Absolute value, Root Mean-Squared Error.
...
kable(model_performance, format = 'pipe')
```
